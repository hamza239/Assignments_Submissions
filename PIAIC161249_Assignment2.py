...
def function6():
    ...
    arr= np.zeros(10);arr[5]=10;arr[8]=20
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.stack (  (np.stack((arr,arr,arr), axis=1) , np.stack((arr,arr,arr), axis=0)),axis=0 ).flatten()
...
def function17():
    ...
    return np.where((arr%5==0)&(arr%7==0), 'YES','NO')
...
def function18():
    ...
    x = np.count_nonzero(np.in1d(piaic, students))
    return x
